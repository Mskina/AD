...
//		profe.getModulos().add(mod);
		...
		profe.addModulo(mod); // igual a mod.addProfesor(profe);
		...
		/*
		 * --declare @idProfe int = 1000 --delete from profesormodulo where
		 * idProfesor=@idProfe --delete from profesor where id=@idProfe --delete from
		 * modulo where nombre='fol'
		 */
	...
	private static void crearComunidadYProvincias(Session session) {
		...
		try {
			...
			prov1.setNombre("Nueva prov 3.1");
			// El id no se asigna automáticamente en esta tabla
			prov1.setIdProvincia(idCA + 1);

			Provincia prov2 = new Provincia();
			prov2.setNombre("Nueva prov 3.2");
			// El id no se asigna automáticamente en esta tabla
			...
			// Se establecen las relaciones bidireccionales entre Provincia y CA
...
//			prov2.setComunidadAutonoma(ca);
			...
			// Primero guardamos el extremo 1 (CA)
			session.save(ca);

			// Después el extremo N (Provincias)
//			session.save(prov1);
//			session.save(prov2);
...
//						prov.setComunidadAutonoma(null);
				...
				System.out.println("No se puede eliminar la Provincia con id: " + idCA + ". No existe");
			}
