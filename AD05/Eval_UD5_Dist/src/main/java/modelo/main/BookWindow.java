...
public class BookWindow extends JFrame {
	...
	private IServicioBook servicioBook;
	...
	public BookWindow() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					List<Book> bookList = servicioBook.findAll();
					addMensaje(true, "Se han recuperado: " + bookList.size() + " libros");

					DefaultListModel<Book> defModel = new DefaultListModel<>();
					for (Book b : bookList) {
						defModel.addElement(b); // Inicialmente usaba add pero le tenía que dar el orden y se colocaban
												// de mayor a menor
					} // https://docs.oracle.com/javase/7/docs/api/javax/swing/DefaultListModel.html
					JListBooks.setModel(defModel);

				} catch (Exception ex) {
					addMensaje(true, "Ha ocurrido un error al listar los libros");
				}
